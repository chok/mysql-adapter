...
function initializeConnection(connection, schema, callback) {
    ...
    function initDatabase() {
        if (typeof schema.settings.database !== 'undefined') {
...
MySQL.prototype.query = function (sql, callback, quiet) {
    ...
    function process(err, c) {
        if (err) {
            return checkData(err);
        }

        connection = c;
        connection.query(sql, checkData)
    }

    if(pool) {
        this.client.getConnection(process);
    } else {
...
MySQL.prototype.all = function all(model, filter, callback) {
    ...
    this.query(sql, function (err, data) {
        ...
        if (filter && filter.attributes) {
            if (!Array.isArray(filter.attributes) && attributes.length == 1) {
                var valueArray = []
                data.forEach(function (value, index) {
                    valueArray.push(value[attributes[0]]);
                });
                objs = valueArray;
            } else {
                objs = data;
