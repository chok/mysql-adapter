...
function initializeConnection(connection, schema, callback) {
    var s = schema.settings;
    var silentOnError = s.silentOnError || false;
    var retryOnError = s.retryOnError || true;
    var createDatabaseOnError = s.createDatabaseOnError || true;
...
function getConnection(settings) {
    ...
    for (var key in options) {
        ...
        if (setting !== null) {
            connectionSettings[option] = setting;
        }
    }
